    ...
    def _check_submitted_signatures(self):
        for record in self:
            if record.state == "submitted" and (
                not record.driver_signature
                or not record.driver_signer_id
                or not record.driver_signature_time
                or not record.warehouse_signature
                or not record.warehouse_signer_id
                or not record.warehouse_signature_time
            ):
                raise ValidationError(_("Driver and warehouse operator handwritten signatures are required."))
    ...
    def action_submit(self):
        for record in self:
            ...
            if not record.driver_signature or not record.warehouse_signature:
                raise UserError(_("Driver and warehouse operator handwritten signatures are required."))
            record.write({
                "state": "submitted",
