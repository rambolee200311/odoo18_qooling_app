...
当前两个翻译目录各登记 `289` 条用户可见字符串，已翻译 `289` 条，
目录覆盖率为 `100%`。这里的覆盖率指当前目录中已登记的字符串；新增
用户可见字符串仍必须同步补充翻译。
...
以上限制由 [媒体证据 Coding Contract](docs/context/intent/Coding_Contract_PDA_Media_Evidence.md)
定义，当前为代码中的硬限制，不提供系统参数配置。当前版本也不支持离线填报。
