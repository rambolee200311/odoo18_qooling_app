...
| Web | 填写、保存、提交、查看和复核 |
| PDA | 通过专门的触控 Web 界面填写、保存、提交和绘制手写签名 |
...
| 字段 | 类型/选择值 | 必填和说明 |
|---|---|---|
| Photos | 多张图片 | 非必填，可与表单记录关联；每张图片可删除或放大查看 |
| Comments | 文本 | 非必填 |
...
#### FR-INBOUND-09 PDA 触控 Web 录入

PDA 可以通过专门的触控 Web 界面填写、保存和提交 Inbound 记录，并支持直接绘制手写签名。
...
| v0.8.0 | 2026-09-22 | 明确 PDA 需要专门的触控 Web 界面 | Agent |
| v0.9.0 | 2026-09-22 | 将温度控制名称和每日记录频率明确为用户决策，不列为待确认项 | Agent |
