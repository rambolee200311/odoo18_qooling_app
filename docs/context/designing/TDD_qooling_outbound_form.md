...
> 基线日期：2026-09-22
> 冻结日期：2026-09-23
> 上游 SRS：[SRS_qooling_outbound_form.md](./SRS_qooling_outbound_form.md) `v1.1.0`
...
| `driver_signature` | Binary | 手写签名，非必填证据 |
...
| `warehouse_signature` | Binary | 手写签名，非必填证据 |
...
| ORM-OUTBOUND-052 | 签名存在时保存对应图像及审计字段；签名缺失不阻止 `state = submitted` |
...
| SVC-OUTBOUND-003 | 提交 | 校验时间和必填字段；签名可选，存在时写入对应审计字段 |
...
| 编号 | 场景 | 处理 |
|---|---|---|
| ERR-OUTBOUND-001 | 缺少必填字段 | 显示用户可见校验错误并保持草稿；签名缺失不报错 |
