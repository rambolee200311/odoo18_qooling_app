...
| Driver comments | 非必填 |
| Driver signature agreement | 手写签名，必填；缺少时不能保存 |
| Photo of cargo | 非必填，不作为提交阻断条件 |
| Warehouse operator comments | 非必填 |
| Warehouse operator signature | 手写签名，必填；缺少时不能保存 |
...
司机和仓库操作员手写签名为必填；其他检查是否阻止提交由人工复核决定，系统只记录结果。
...
已提交记录可由授权用户撤回至草稿。司机或仓库操作员缺少手写签名时不能保存为完成或提交。
...
- 检查项不合格是否阻止出库放行由人工复核决定；
- 重量分布的判断标准由人工复核决定；
- 温度范围和异常阈值由人工复核决定，系统只记录用户结果。
